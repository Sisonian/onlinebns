...
<link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet">
	...
	<div id="mySidenav" class="sidenav">
     ...
     <div class="main">
      <div style="text-align: center; background: #F2F2F2; padding-top:18px;"><h1>You Searched for : "<?php  
      $searchname = mysqli_real_escape_string($connect,$_POST['searchbox']);
      echo "$searchname"; 
      ?>"</h1><br></div>
...
{          
    ...
    {        
        $search = mysqli_real_escape_string($connect,$_POST['searchbox']);
        $userSQL = mysqli_query($connect,"SELECT * FROM items where (seller_name LIKE '%".$search."%' OR item_name LIKE '%".$search."%' OR item_condition LIKE '%".$search."%' OR seller_location LIKE '%".$search."%' OR item_category LIKE '%".$search."%') AND approved_to_post='Approved'");
        $numResults = mysqli_num_rows($userSQL);
        if (!$numResults)
{
            echo "<center><h1>There's no Item that matches your search</h1></center>";
}
else
{
